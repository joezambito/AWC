...
struct WealthSystemHelpSection: View {
    ...
    private var helpCards: some View {
        ...
        wealthSystemHelpCard(title: "CARD COLOR", rows: ["Both AI score and confidence must be green before AI can buy", "If AI score and confidence are mixed, the card box goes purple", "If both are the same color, the card box uses that color", "Cards update color again when new data changes AI score or confidence"], tint: WealthTheme.orange)
        ...
        wealthSystemHelpCard(title: "ACTIVITY STATUS", rows: ["Waiting for Trade = order is ready when the next tradable session opens", "Buy Submitted / Buy Pending = buy is in progress", "Sale Submitted / Sale Pending = sell is in progress", "Completed = the order filled and will move out of Activity"], tint: WealthTheme.purple)
